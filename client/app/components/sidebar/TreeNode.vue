  ...
  <li class="relative pl-2 group">
    ...
    >
      ...
      <button
        ...
        :aria-expanded="app.treenode.open"
      >
        <Icon
          :name="app.treenode.open ? 'heroicons:chevron-down' : 'heroicons:chevron-right'"
        ...
        <span v-if="app.treenode.loading" class="ml-2 animate-pulse text-xs text-gray-400">
          Loading...
        </span>
      </button>
      ...
      <div
        v-else
        @click="app.sidebar.setActive(node.path, false)"
        :class="{
          'bg-blue-500 text-white': app.sidebar.activePath === node.path,
          'bg-blue-100': app.sidebar.activePath.startsWith(node.path + '/')
        }"
      >
    ...
    <transition name="fade" mode="out-in">
      <ul
        v-if="node.type === 'folder' && node.children && node.children.length && app.treenode.open"
