<template>
<div
  :class="['flex flex-col h-dvh bg-white dark:bg-gray-900 shadow-lg transition-all duration-300 ease-in-out relative  min-w-0', collapsed ? 'w-18' : 'w-64']"
>    
    <!-- Header -->
    <div class="flex items-center pl-2 h-[60px] border-b border-gray-200 dark:border-gray-700 flex-shrink-0 gap-2" :class="collapsed ? 'justify-center' : ''">
      <img :src="app.darkmode ? logoDark : logoLight" alt="SwiftFiles.io's Logo" class="h-3/4 w-auto object-contain" />
      <h3 v-if="!collapsed" class="text-md text-gray-900 dark:text-gray-100">SwiftFiles.io</h3>
    ...
    <!-- Content -->
   <!-- Content -->
<div class="flex-1 overflow-auto p-4" v-show="!collapsed">
...
<div class="flex-1  p-4" v-show="collapsed">
  <ul class="space-y-4 text-sm text-gray-600 dark:text-gray-300">
    
    <li @click="collapsed = !collapsed" class="flex items-center justify-center gap-4 cursor-pointer text-gray-500 hover:text-blue-500 transition relative group">
    ...
    <div class="p-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0 flex items-center justify-between" :class="collapsed ? 'flex-col gap-3' : ''">
      <div v-if="!collapsed" @click="toggleDarkMode" class="w-14 h-7 flex items-center rounded-full cursor-pointer transition-colors duration-300 ease-in-out relative" :class="app.darkmode ? 'bg-gray-700' : 'bg-gray-300'">
      ...
      <div v-else @click="toggleDarkMode" class="h-7 flex items-center rounded-full cursor-pointer transition-colors duration-300 ease-in-out relative" >
      ...
      @click="collapsed = !collapsed"
        ...
        <Icon 
          ...
          :name="collapsed ? 'heroicons:arrow-long-right' : 'heroicons:arrow-long-left'" 
