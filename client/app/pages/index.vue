<template>
  <div class="flex flex-col h-dvh w-74 bg-white dark:bg-gray-900 shadow-lg transition-colors duration-300 ease-in-out">
    ...
    <div class="flex items-center justify-center h-18 border-b border-gray-200 dark:border-gray-700 flex-shrink-0 gap-2">
      <img :src="darkMode ? logoDark : logoLight" alt="SwiftFiles.io's Logo" class="h-12 w-auto object-contain" />
...
<div class="flex-1 overflow-auto p-4">
  ...
  <ul class="space-y-2 text-sm text-gray-600 dark:text-gray-300">
    ...
    <div class="p-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0 flex items-center justify-center">
      <div @click="toggleDarkMode" class="w-14 h-7 flex items-center rounded-full cursor-pointer transition-colors duration-300 ease-in-out relative" :class="darkMode ? 'bg-gray-700' : 'bg-gray-300'">
        <Icon name="heroicons:sun" class="absolute left-1 w-4 h-4 text-yellow-500 opacity-70 transition-opacity duration-300" :class="darkMode ? 'opacity-30' : 'opacity-70'" />
        <Icon name="heroicons:moon" class="absolute right-1 w-4 h-4 text-gray-400 opacity-70 transition-opacity duration-300" :class="darkMode ? 'opacity-70' : 'opacity-30'" />
        <span class="w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-300 ease-in-out flex items-center justify-center" :class="darkMode ? 'translate-x-7' : 'translate-x-0'">
          <Icon :name="darkMode ? 'heroicons:moon' : 'heroicons:sun'" class="w-3 h-3 text-gray-600" />
        </span>
      </div>
    </div>
  </div>
